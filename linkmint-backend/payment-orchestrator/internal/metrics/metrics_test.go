...
func TestRecordersAndScrape(t *testing.T) {
	m := New()
	m.ChainEvent("paylink.verified", "applied")
	m.Transition("AWAITING_PAYMENT", "SETTLED")

	// Drive the HTTP middleware once.
	h := m.Middleware(func(*http.Request) string { return "/v1/payments" })(
		http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusCreated) }))
	h.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodPost, "/v1/payments", nil))

	rr := httptest.NewRecorder()
	m.Handler().ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/metrics", nil))
	body := rr.Body.String()
	for _, want := range []string{"chain_events_processed_total", "payment_transitions_total", "http_requests_total"} {
