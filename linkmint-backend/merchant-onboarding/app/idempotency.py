...
A request that re-presents the same key+body gets the cached response (replay). The same key with
a *different* body is a conflict (409). A key whose first request is still in flight is also a
conflict. Keys are namespaced per route so onboard/add-bank-account/etc. never collide.

NOTE: the cached *body* is whatever the route stores. Routes that handle secrets (bank account
add) deliberately exclude raw plaintext from both the fingerprint payload and the cached body —
the plaintext account details are never written here.
"""
...
class IdempotencyStore:
    ...
    @staticmethod
    def _key(route: str, key: str) -> str:
        return f"idem:merchant-onboarding:{route}:{key}"
    ...
    async def begin(self, route: str, key: str, fp: str) -> CachedResponse | None:
        ...
        Raises :class:`AppError` (409) on a body mismatch or an in-flight duplicate.
        """
