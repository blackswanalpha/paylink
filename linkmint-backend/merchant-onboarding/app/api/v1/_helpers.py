"""Shared idempotency wrapper for state-mutating endpoints (mirrors identity-service's pattern)."""
...
async def idempotent(
    ...
    SECURITY: ``payload`` (fingerprint input) and the body ``work`` returns must NOT contain raw
    secrets — the bank-account route passes a redacted fingerprint and returns ids/status only.
    """
