...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # work15 — outbox-drain relay (drains the merchant_events outbox to the bus). Lazily imported so
    ...
    # work15 — bus consumer (subscribes to the bus → MerchantEventConsumer.handle). Lazily imported;
    # runs only when MERCHANT_EVENT_CONSUMER_ENABLED=true (all app.state is set by now).
