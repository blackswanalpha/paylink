...
def test_upgrade_from_empty_then_downgrade() -> None:
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            assert {"paylinks", "paylink_events"} <= tables

            indexes = set(
                conn.execute(
                    sa.text("SELECT indexname FROM pg_indexes WHERE schemaname = 'paylink'")
                ).scalars()
            )
            assert {
