...
conflict. Keys are namespaced per route so different endpoints never collide.

NOTE: this dedupes *HTTP retries of the same request*. The separate per-event ``dedupe_key`` guard
in :mod:`app.domain.service` is what stops an at-least-once event bus from double-notifying.
"""
...
class IdempotencyStore:
    ...
    def _key(route: str, key: str) -> str:
        return f"idem:notification-service:{route}:{key}"
