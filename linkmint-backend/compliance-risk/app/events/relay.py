...
The domain writes an event row into the durable outbox table in the same transaction as the business
change (the source of truth). This background task drains unpublished rows to the Kafka bus via
``linkmint_eventbus`` and marks them published. It runs only when ``EVENT_PUBLISHER_MODE=kafka``.
...
log = get_logger("compliance.relay")
