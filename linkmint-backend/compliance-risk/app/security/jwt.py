"""RS256 access-token verification (PyJWT) — compliance-risk is a CONSUMER, not an issuer.

Unlike identity-service (which signs and verifies its own tokens), compliance-risk only *verifies*
identity-service's RS256 access tokens with identity's public key. There is intentionally NO
``JwtIssuer``/``KeyStore`` here. The verifier is RS256-only — it never accepts HS256 or ``none``
(alg-confusion guard): the RS256 signature is the trust anchor for the ``sub``/role claims this
service authorizes from (self-or-admin on the KYC/status surface).
"""
    ...
    def verify(self, token: str) -> AccessClaims:
        try:
            payload = jwt.decode(
                ...
                # (defense-in-depth; identity-service always mints exp).
                options={"require": ["exp"]},
            )
