...
from tests._support import AUDIENCE, ISSUER, TEST_PRIVATE_PEM, TEST_PUBLIC_PEM, mint_token
...
def test_verify_roundtrip() -> None:
    token = mint_token(user_id="u1", roles=[("o1", "admin")], user_roles=["payer"], kyc_tier=2)
    claims = _verifier().verify(token)
    assert claims.user_id == "u1"
    assert claims.kyc_tier == 2
    assert claims.roles[0].org_id == "o1" and claims.roles[0].role == "admin"
...
def test_token_without_exp_rejected() -> None:
    # A token signed by the trusted key but MISSING `exp` must not be treated as non-expiring
    # (the verifier requires `exp`). PyJWT raises MissingRequiredClaim → InvalidToken.
    payload = {"sub": "u1", "iss": ISSUER, "aud": AUDIENCE, "user_roles": ["payer"], "kyc_tier": 1}
    token = pyjwt.encode(payload, TEST_PRIVATE_PEM, algorithm="RS256")
    with pytest.raises(AppError) as exc:
