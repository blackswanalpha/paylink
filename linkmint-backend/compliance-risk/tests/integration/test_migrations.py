...
def test_upgrade_from_empty_then_downgrade() -> None:
    ...
    try:
        ...
        os.environ["COMPLIANCE_DATABASE_URL"] = url
        ...
        with engine.connect() as conn:
            ...
            indexes = set(
                conn.execute(
                    sa.text("SELECT indexname FROM pg_indexes WHERE schemaname = 'compliance'")
                ).scalars()
            )
            assert {
                "ix_risk_scores_user_id",
                "ix_flags_user_id",
        ...
        with engine.connect() as conn:
            ...
            assert "kyc_records" not in tables
        engine.dispose()
    finally:
        container.stop()
