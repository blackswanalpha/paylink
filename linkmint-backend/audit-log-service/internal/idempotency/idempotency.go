...
//
// For audit-log intake the key is OPTIONAL (an audit signal must never be dropped for a missing
// header); when present it dedupes producer retries via this store. See server.postEntry.
package idempotency
