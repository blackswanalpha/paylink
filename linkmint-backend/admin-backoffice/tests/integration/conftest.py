...
from app.deps import get_app_services
...
@pytest.fixture
def live_client(pg_url: str, redis_url: str) -> Iterator[tuple[TestClient, CapturingAuditSink]]:
    """A TestClient over the REAL Postgres + Redis (real staff repo + readyz), with in-memory
    providers (no upstreams running). Yields the client + the capturing audit sink."""
    settings = make_settings(
        database_url=pg_url, redis_url=redis_url, jwt_public_key_pem=TEST_PUBLIC_PEM
    )
    app = create_app(settings)
    audit = CapturingAuditSink()
    with TestClient(app) as client:
        services = build_services(fake_registry(), audit, settings)  # type: ignore[arg-type]
