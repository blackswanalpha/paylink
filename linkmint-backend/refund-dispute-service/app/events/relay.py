...
The domain writes an event row into ``refund.refund_events`` in the same transaction as the business
change (the source of truth). This background task drains unpublished rows to the Kafka bus via
``linkmint_eventbus`` and marks them published. Runs only when ``EVENT_PUBLISHER_MODE=kafka``.
